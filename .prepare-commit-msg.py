...
append = False
if len(branch.split('_')) != 0:
    ticket_number = branch.split('_')[-1] 
    if ticket_number != '' and ticket_number is not None:
        if len(ticket_number.split('-')) >= 2:
            number = ticket_number.split('-')[1]
            if number.isdigit():
                append = True
...
if append:
    ...
    # ticket_number = branch.split('_')[-1]
    print("prepare-commit-msg: Prepending [%s] to commit message" % ticket_number)

    with open(commit_msg_filepath, "r+") as f:
        content = f.read()
        f.seek(0, 0)
        f.write("[%s] %s" % (ticket_number, content))
else:
